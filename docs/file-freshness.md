...
The command reports the resolved path, file size, UTC modification timestamp, current age, configured maximum age, and an `ok` or `stale` state. `--json` and `--csv` provide machine-readable output through the same reporting layer as other AutoOPS checks.

Exit code `0` means the check completed successfully. With `--fail-on-stale`, exit code `1` means the file is older than the configured threshold while still preserving the report for CI or monitoring artifacts. Exit code `2` indicates an invalid threshold, missing path, non-file path, or filesystem error.
